    ...
    def segment(self, volume):
        ...
        if self.model_name == SegModel.TIRAMISU_ACDC.value:
            ...
            if self.edge_device == EdgeDevice.RASBERRY_PI.value:
                ...
                # model_path = os.path.join(os.path.dirname(__file__), "../Resources/tiramisu_emidec_fp16.onnx")
                # self.model = torch.jit.load(model_path)
                # self.fp16 = True
                provider = ["CUDAExecutionProvider", "CPUExecutionProvider"]
                self.model = ort.InferenceSession(model_path, providers=provider)
        ...
        if self.model_name == SegModel.TIRAMISU_ACDC.value:
            num_class = 4
            seg_array = predict_data_model(data, self.model, num_classes=num_class, 
                                           min_size_remove=800, fp16=self.fp16, device=self.device).astype(np.uint8)
            if self.myo_only:
