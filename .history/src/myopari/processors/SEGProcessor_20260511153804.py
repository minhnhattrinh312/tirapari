...
class SEG_module():
    ...
    def segment(self, volume):
        ...
        self.fp16 = False
        self.onnx = True
        data = preprocess_data_array(volume, onnx=self.onnx)
        if self.model_name == SegModel.TIRAMISU_ACDC.value:
            num_class = 4
            seg_array = predict_data_model(data, self.model, num_classes=num_class, min_size_remove=800, 
                                           fp16=self.fp16, onnx=self.onnx).astype(np.uint8)
            if self.myo_only:
                seg_array[seg_array != 2] = 0
            return seg_array
        elif self.model_name == SegModel.TIRAMISU_EMIDEC.value:
            num_class = 5
            seg_array = predict_data_model_emidec(data, self.model, fp16=self.fp16, onnx=self.onnx).astype(np.uint8)
            if self.myo_only:
